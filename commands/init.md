...
7. Point the user to `docs/WORKFLOW.md`. **Engine setup is separate** (Superset — ADR-0002 Update): download the macOS app from `github.com/superset-sh/superset/releases/latest` and open it; it runs each agent in its own worktree over your logged-in CLIs. (Per worktree, deps bootstrap fast with pnpm — `corepack enable && pnpm install --frozen-lockfile` — since pnpm hardlinks from its global store, not a full `node_modules` copy.)
