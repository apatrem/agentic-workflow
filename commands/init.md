...
8. Point the user to `docs/WORKFLOW.md`. **Engine setup is separate** (Superset — ADR-0002 Update): install the macOS app (`github.com/superset-sh/superset/releases/latest`) and/or the `superset` CLI; it runs each agent in its own worktree over your logged-in CLIs — interactively or via `superset agents create` (see `/agentic-workflow:run`).
