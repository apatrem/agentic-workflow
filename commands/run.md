...
The engine is **Composio (`ao`)**, not a bespoke harness (ADR-0002). This command preps and hands off.
...
2. **Engine up:** `ao start` (dashboard on :3000). Confirm `agent-orchestrator.yaml` — its `worker.agent` (default `cursor` = Cursor Composer — ADR-0002 Update; the separate `orchestrator` role stays `claude-code`) and the gate in `agentRules` match this repo.
...
5. Open a PR → CI runs the gate → **sparse review** (blockers only) → **a human merges**. Autonomous auto-merge is the advanced tier (ADR-0003), not the default.
