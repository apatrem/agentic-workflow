...
| "It's a tiny/obvious change — skip review." | The risk floor is structural, not a judgement call: destructive-or-protected **or governance** surface forces **≥ medium** regardless of how small it looks (AW-0004). | risk floor |
...
| "Re-spawn keeps failing — I'll just fix it inline." | The orchestrator **never implements inline** (context-rot, §1). Fall back to the direct-CLI-in-worktree path (§5) and stay thin. | thin orchestrator |
| "Round 3 didn't clear it, but I'm close — one more round." | The cap is **3 review rounds → `needs-human`** (AW-0006). Hand the PR over with the open blockers flagged; don't auto-loop past the budget. | round cap |
