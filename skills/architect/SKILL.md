...
- **Low- vs high-fidelity.** Grilling settles *low-fidelity* decisions (data shapes, trade-offs, conventions,
  ...
  grilling can't settle. *(A **spike** = a throwaway branch: no frozen tests, explore only until the question is
  answered, then **discard the code** and carry just the answer back into the grill — never merge a spike.)*
