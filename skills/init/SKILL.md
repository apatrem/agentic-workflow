...
Set the current repo up to follow the workflow (AW-0001). Do **not** overwrite existing files; report and skip.
...
6. **Node setup — standardize on pnpm (Corepack — AW-0009):** if this is (or will be) a Node repo, write `package.json` from `templates/package.template.json` with `packageManager: "pnpm@<version>"` so every clone, worktree, and CI run uses the same pnpm. Commit `pnpm-lock.yaml`; do **not** also commit `package-lock.json` / `yarn.lock`. (Skip for non-Node repos.)
...
8. Point the user to `docs/WORKFLOW.md`. **Engine setup is separate** (Superset — AW-0002 Update): install the macOS app (`github.com/superset-sh/superset/releases/latest`); the bundled CLI lives at `~/.superset/bin/superset`. Workers spawn via `superset workspaces create … --agent … --prompt …` (see `/agentic-workflow:run`). When configuring presets, give each its CLI's auto-approve flag (e.g. cursor `--force`) so headless spawns don't block on approval prompts — see the `/agentic-workflow:run` pre-flight.
