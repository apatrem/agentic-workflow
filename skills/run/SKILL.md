...
The engine is **Superset** (AW-0002 Update) — a macOS app **and** a headless **CLI / SDK / MCP server**. Each
...
> spawn, per tier), **not** the engine's API — external (AW-0002 / 0007) and versioned. Commands below verified
...
> spawn-reliability fallbacks, and the **anti-rationalization table** (§8 — the excuses that skip the gate /
verify / review / cleanup, and the rebuttals) — is in **`docs/ORCHESTRATOR_PLAYBOOK.md`**.
...
1. **Pre-flight:** `main` clean + protected; the agent CLIs (`claude` / `codex` / `cursor-agent`) logged in on
   ...
   (AW-0002), and it must carry its CLI's auto-approve flag or the worker blocks on the first approval prompt
   and the spawn never returns. Flags (verify against each CLI's `--help`; external/versioned per AW-0002):
...
2. **Spawn the worker(s)** per the task's **`mode`** (AW-0004) — each gets its own worktree + the right model.
   **Enforce the risk floor before spawning:** if the task's *Files likely involved* / acceptance touch destructive-or-protected surface scoped in by a human (destructive fs ops, gate/CI, lockfiles/deps, migrations, auth, public APIs) — or **governance / decision-record surface** (`docs/adr/*`, `CONTEXT.md`, `docs/MODELS.md` role→model assignments, the conventions/skills/templates; routine prose and `MODELS.md` benchmark/date upkeep stay `low`) — and its `mode` is below the floor, **bump it** (`low`→`medium`; `medium`→`hard` if the change is also large or ambiguous) and note *"escalated by risk floor"* on the PR (AW-0004 refinement 3) — don't spawn a `low` run on that surface.
   Spawn in **two steps — the reliable default** (the one-call form can race; playbook §2). Wrap the task file in an orchestrator preamble — never pass it bare:
   ```
   # verified against superset v0.2.19 — confirm with `superset --help`
   # 1) create the worktree WITHOUT --agent, capturing its id (--quiet prints just the workspace id;
   #    --base-branch defaults to the project default)
   ws=$(superset workspaces create --local --project prj_… --name <task> \
        --branch agent/<lineage>/<task> --quiet)
   # 2) spawn the agent into the ready workspace. <lineage> maps to a Superset preset id —
   #    cursor → cursor-agent, codex → codex, claude → claude (confirm with `superset agents list`).
   superset agents create --workspace "$ws" --agent <preset> --prompt "$(cat /tmp/<task>-prompt.txt)"
   # Optional shortcut — one call, fine when it lands; if the agent doesn't materialize (silent race),
   # fall back to the two steps above:
   #   superset workspaces create … --agent <preset> --prompt "$(cat /tmp/<task>-prompt.txt)"
   ```
   ...
   - **`hard`** — best-of-N over **two authoring lineages** (`--agent cursor` + `--agent codex`; bias each per
     ...
     authoring and synthesis**, so it stays the *structurally-clean* reviewer (AW-0004 invariant). Then
...
4. Push the chosen branch; open a PR via `gh` → CI re-runs the gate. Once CI is green, review per tier
   ...
   - **`low`** — one **adversarial reviewer** (≤10 ranked findings), a lineage **independent of the
     ...
     AW-0011); it stays advisory, the veto is blockers-only.
   ...
   If review raises blockers, run the **remediation loop** (AW-0010; mechanics in playbook §6): **remediate
   ...
   **seed candidate #0**, runs best-of-N in parallel, and smart-merges {seed + attempts}. Cap: **3 full review
   ...
   (AW-0006). Then **a human merges**. **smart-merge ≠ auto-merge**; autonomous auto-merge is the separate
   advanced tier (AW-0003 / 0008).
